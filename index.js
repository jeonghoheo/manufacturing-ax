...
function createWorkStep(targetOperation) {
    const targetRouting = routing.find((item) => item.operation === targetOperation);
    if (!targetRouting) {
        throw new Error(` ${targetOperation} Routing을 찾을 수 없습니다.`);
    }
    const targetPart = bom.find((item) => item.partNumber === targetRouting.partNumber);
    if (!targetPart) {
        throw new Error(`${targetRouting.partNumber} 부품을 BOM에서 찾을 수 없습니다.`);
    }
